...
namespace CefSharp.MinimalExample.WinForms
{
    public class CustomRequestHandler : IRequestHandler
    {
        ...
        CefReturnValue IRequestHandler.OnBeforeResourceLoad(IWebBrowser browserControl, IBrowser browser, IFrame frame, IRequest request, IRequestCallback callback)
        {
            ...
            if (!callback.IsDisposed)
            {
                using (callback)
                {
                    ...
                    request.Url = request.Url.Replace("http://foo.web.com", "http://localhost:9696");
        ...
        void IRequestHandler.OnResourceRedirect(IWebBrowser browserControl, IBrowser browser, IFrame frame, IRequest request, IResponse response, ref string newUrl)
        {
            ...
            //}
            if (request.Url.StartsWith("http://foo.web.com") && !newUrl.StartsWith("http://localhost:9696"))
            {
                newUrl = request.Url.Replace("http://foo.web.com", "http://localhost:9696");
            }
        }
